...
function change_ip_for_debian(){


      echo -e -n "\n1. Use static IP adress\n"
      echo -e -n "2. Use dynamic IP adress\n\n"
      read -r -p "Enter the action you want to do: " choice1

                  case $choice1 in
                  1)
                        sudo sed -i "/ethernets:/,$ d" /etc/netplan/*.yaml                   # delete all strings after "ethernets"
                        echo -e -n "\n\nYour default gateway now is:\n"
                        ip rout | grep default
                        echo -e -n "\nEnter your default gateway: "
                        read gateway1
                        check_ip $gateway1;
                        read -r -p "Enter your static ip adress: " ip1
                        check_ip $ip1;
                        read -r -p "Enter your network mask (/24, /16 etc.): " netmask1
                        #check_ip $netmask1;
                        read -r -p "Enter DNS server (recommended 8.8.8.8): " dns11
                        check_ip $dns11;
                        read -r -p "Enter reserve DNS server (recommended 8.8.4.4): " dns22
                        check_ip $dns22;

                        echo "  ethernets:" >> /etc/netplan/*.yaml
                        net_interface="$(ip addr | grep 2: | grep -Eo "en.*:" )"
                        echo "    $net_interface" >> /etc/netplan/*.yaml
                        echo "      dhcp4: false" >> /etc/netplan/*.yaml
                        echo "      addresses: [$ip1$netmask1]" >> /etc/netplan/*.yaml
                        echo "      gateway4: $gateway1" >> /etc/netplan/*.yaml
                        echo "      nameservers:" >> /etc/netplan/*.yaml
                        echo "        addresses: [$dns11,$dns22]" >> /etc/netplan/*.yaml
                  ...
                  2)
                        ...
                        sudo sed -i "/ethernets:/,$ d" /etc/netplan/*.yaml
                        sudo echo "  ethernets:" >> /etc/netplan/*.yaml
                        net_interface1="$(ip addr | grep 2: | grep -Eo "en.*:" )"
                        echo "    $net_interface1" >> /etc/netplan/*.yaml
                        echo "      dhcp4: true" >> /etc/netplan/*.yaml
                        echo "  version: 2" >> /etc/netplan/*.yaml
...
os="$(cat /etc/os-release | grep -Eo "ID_LIKE=.*")"

if [[ $os == "ID_LIKE=\"rhel fedora\"" || $os == "ID_LIKE=rhel fedora" ]]; then
change_ip_for_fedora;
fi

if [[ $os == "ID_LIKE=\"debian\"" || $os == "ID_LIKE=debian" ]]; then
change_ip_for_debian;
fi
